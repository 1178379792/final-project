...
@Service
public class TushuxinxiService {

    @Resource
    private TushuxinxiMapper mapper;

    public ResponseData<Tushuxinxi> findById(Integer id) {
        return JsonResult.success(mapper.selectById(id));
    }

    public ResponseData<List<Tushuxinxi>> selectAll(Wrapper<Tushuxinxi> query) {
        return JsonResult.success(mapper.selectList(query));
    }

    public ResponseData<List<Tushuxinxi>> selectAll() {
        QueryWrapper<Tushuxinxi> wrapper = Wrappers.query();
    ...
    public ResponseData selectAll(Map<String, Object> map) {
        ...
        QueryWrapper<Tushuxinxi> wrapper = mapToWrapper(map);
    ...
    public QueryWrapper<Tushuxinxi> mapToWrapper(Map<String, Object> map) {
        QueryWrapper<Tushuxinxi> wrapper = Wrappers.query();

        String where = " 1=1 ";
        if (!StringUtil.isNullOrEmpty(map.get("keyword"))) {
            wrapper.apply(" ( mingcheng LIKE '%" + map.get("keyword") + "%' OR zuozhe LIKE '%" + map.get("keyword") + "%' OR chubanshe LIKE '%" + map.get("keyword") + "%' ) ");
        }
        ...
        if (!StringUtil.isNullOrEmpty(map.get("zuozhe"))) {
            wrapper.like("zuozhe", map.get("zuozhe"));
        }
        if (!StringUtil.isNullOrEmpty(map.get("chubanshe"))) {
            wrapper.like("chubanshe", map.get("chubanshe"));
        }
     ...
     * @return 搜索到得数据
     */
    public ResponseData<List<Tushuxinxi>> filter(Map<String, Object> map) {
        Class<Tushuxinxi> aClass = Tushuxinxi.class;
        com.jntoo.db.QueryWrapper<Tushuxinxi> wrapper = DB.name(aClass);
    ...
    public ResponseData selectPages(QueryWrapper<Tushuxinxi> wrapper, IPage page) {
        Map result = new HashMap();
        result.put("lists", mapper.selectPage(page, wrapper));
        return JsonResult.success(result);
    }

    public ResponseData insert(Tushuxinxi entityData, Map post) {
    ...
    public ResponseData<Object> update(Tushuxinxi entityData, Map post) {
