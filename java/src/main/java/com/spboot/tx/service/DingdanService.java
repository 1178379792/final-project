...
public class DingdanService {
    ...
    public ResponseData<Dingdan> findById(Integer id) {
        return JsonResult.success(mapper.selectById(id));
    }

    public ResponseData<List<Dingdan>> selectAll(Wrapper<Dingdan> query) {
    ...
    public QueryWrapper<Dingdan> mapToWrapper(Map<String, Object> map) {
        ...
        if (!StringUtil.isNullOrEmpty(map.get("dingdanbianhao"))) {
            wrapper.like("dingdanbianhao", map.get("dingdanbianhao"));
        }
        if (!StringUtil.isNullOrEmpty(map.get("dingdanxinxi"))) {
            wrapper.like("dingdanxinxi", map.get("dingdanxinxi"));
        }
     ...
     * @return 搜索到得数据
     */
    public ResponseData<List<Dingdan>> filter(Map<String, Object> map) {
        Class<Dingdan> aClass = Dingdan.class;
        com.jntoo.db.QueryWrapper<Dingdan> wrapper = DB.name(aClass);
    ...
    public ResponseData selectPages(QueryWrapper<Dingdan> wrapper, IPage page) {
        Map result = new HashMap();
        result.put("lists", mapper.selectPage(page, wrapper));
        return JsonResult.success(result);
    }

    public ResponseData insert(Dingdan entityData, Map post) {
        ...
        if (entityData.getId() != null) {
            ...
            DB.execute(
                "INSERT INTO dingdandingdanxinxi(dingdanid,tushuxinxiid,mingcheng,tupian,fenlei,kucun,jiage,goumaishuliang,xiaoji,goumairen)  SELECT '" +
                charuid +
                "',tushuxinxiid,mingcheng,tupian,fenlei,kucun,jiage,goumaishuliang,xiaoji,goumairen  FROM gouwuche  WHERE goumairen='" +
                Request.user().getUsername() +
                "'"
            );

            DB.execute("DELETE FROM gouwuche WHERE goumairen='" + Request.user().getUsername() + "'");
    ...
    public ResponseData<Object> update(Dingdan entityData, Map post) {
