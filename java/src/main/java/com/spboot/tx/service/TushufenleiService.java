...
@Service
public class TushufenleiService {

    @Resource
    private TushufenleiMapper mapper;

    public ResponseData<Tushufenlei> findById(Integer id) {
        return JsonResult.success(mapper.selectById(id));
    }

    public ResponseData<List<Tushufenlei>> selectAll(Wrapper<Tushufenlei> query) {
        return JsonResult.success(mapper.selectList(query));
    }

    public ResponseData<List<Tushufenlei>> selectAll() {
        QueryWrapper<Tushufenlei> wrapper = Wrappers.query();
    ...
    public ResponseData selectAll(Map<String, Object> map) {
        ...
        QueryWrapper<Tushufenlei> wrapper = mapToWrapper(map);
    ...
    public QueryWrapper<Tushufenlei> mapToWrapper(Map<String, Object> map) {
        QueryWrapper<Tushufenlei> wrapper = Wrappers.query();

        String where = " 1=1 ";
        // 以下是判断搜索框中是否有输入内容，判断是否前台是否有填写相关条件，符合则写入sql搜索语句
        if (!StringUtil.isNullOrEmpty(map.get("fenlei"))) {
            wrapper.like("fenlei", map.get("fenlei"));
        }
     ...
     * @return 搜索到得数据
     */
    public ResponseData<List<Tushufenlei>> filter(Map<String, Object> map) {
        Class<Tushufenlei> aClass = Tushufenlei.class;
        com.jntoo.db.QueryWrapper<Tushufenlei> wrapper = DB.name(aClass);
    ...
    public ResponseData selectPages(QueryWrapper<Tushufenlei> wrapper, IPage page) {
        Map result = new HashMap();
        result.put("lists", mapper.selectPage(page, wrapper));
        return JsonResult.success(result);
    }

    public ResponseData insert(Tushufenlei entityData, Map post) {
    ...
    public ResponseData<Object> update(Tushufenlei entityData, Map post) {
