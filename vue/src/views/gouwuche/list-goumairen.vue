        ...
        <el-card class="box-card">
                ...
                <el-form @submit.prevent.stop :inline="true" size="mini">
                    ...
                    <el-form-item label="分类">
                        <el-select v-model="search.fenlei"
                            ><el-option label="请选择" value=""></el-option><e-select-option type="option" module="tushufenlei" value="id" label="fenlei"></e-select-option
                        ></el-select>
                    </el-form-item>
                    <el-form-item>
            ...
            <el-table border :data="list" style="width: 100%" @sort-change="sortChange" highlight-current-row>
                ...
                <el-table-column prop="fenlei" label="分类" width="80">
                    <template slot-scope="{row}"> <e-select-view module="tushufenlei" :value="row.fenlei" select="id" show="fenlei"></e-select-view> </template>
                </el-table-column>
                <el-table-column sortable="custom" prop="kucun" label="库存" width="80">
                    <template slot-scope="{row}"> {{ row.kucun }} </template>
                </el-table-column>
                <el-table-column sortable="custom" prop="jiage" label="价格" width="80">
                    <template slot-scope="{row}"> {{ row.jiage }} </template>
                </el-table-column>
                <el-table-column sortable="custom" prop="goumaishuliang" label="购买数量" width="80">
                    <template slot-scope="{row}"> {{ row.goumaishuliang }} </template>
                </el-table-column>
                <el-table-column prop="xiaoji" label="小计" width="80">
                    <template slot-scope="{row}"> {{ row.xiaoji }} </template>
                </el-table-column>
                <el-table-column prop="goumairen" label="购买人" width="80">
                    <template slot-scope="{row}"> {{ row.goumairen }} </template>
                </el-table-column>
                    ...
                    <template slot-scope="{row}">
                        <el-button-group>
                            <el-tooltip content="详情" placement="top">
                                <el-button @click="$goto({path:'/admin/gouwuchedetail',query:{id:row.id } })" icon="el-icon-info" type="info" size="mini"></el-button>
                            </el-tooltip>
                            <el-tooltip content="编辑" placement="top">
                                <el-button icon="el-icon-edit" @click="$goto({path:'/admin/gouwucheupdt',query:{id:row.id } })" type="warning" size="mini"></el-button>
                            </el-tooltip>
    ...
    export default {
        ...
        methods: {
            ...
            loadList(page) {
                ...
                filter.session_name = "goumairen";

                this.$store
                    .dispatch("gouwuche/query", filter)
                    .then((res) => {
                        this.loading = false;
            ...
            deleteItems(items) {
                this.$confirm("确定删除数据？此操作不可恢复", "警告", {
                    ...
                    this.$store.dispatch("gouwuche/delete", ids).then((res) => {
                        this.loading = false;
                        if (res.code != api.code.OK) {
