        ...
        <el-card class="box-card">
            ...
            <el-table border :data="list" style="width: 100%" @sort-change="sortChange" highlight-current-row>
                ...
                <el-table-column prop="zhanghao" label="账号" width="130">
                    <template slot-scope="{row}"> {{ row.zhanghao }} </template>
                </el-table-column>
                ...
                <el-table-column prop="shoujihaoma" label="手机号码" width="130">
                    <template slot-scope="{row}"> {{ row.shoujihaoma }} </template>
                </el-table-column>
                <el-table-column prop="shengri" label="生日" width="100">
                    <template slot-scope="{row}"> {{ row.shengri }} </template>
                </el-table-column>
                <el-table-column prop="suozaichengshi" label="所在城市" width="130">
                    <template slot-scope="{row}"> {{ row.suozaichengshi }} </template>
                </el-table-column>
                <el-table-column prop="gexingqianming" label="个性签名">
                    <template slot-scope="{row}"> {{ row.gexingqianming }} </template>
                </el-table-column>
                <el-table-column prop="shouhuodizhi" label="收货地址">
                    <template slot-scope="{row}"> {{ row.shouhuodizhi }} </template>
                </el-table-column>
                    ...
                    <template slot-scope="{row}">
                        <el-button-group>
                            <el-tooltip content="详情" placement="top">
                                <el-button @click="$goto({path:'/admin/yonghudetail',query:{id:row.id } })" icon="el-icon-info" type="info" size="mini"></el-button>
                            </el-tooltip>
