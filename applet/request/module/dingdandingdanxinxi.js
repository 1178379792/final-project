...
var content = (actions = {
    selectAll() { // 获取所有数据
        return new Promise((resolve, reject) => {
            if (content.state.status == 0) {
                ...
                httpGet("/dingdandingdanxinxi/selectAll").then(
                    (res) => {
                        if (res.code == 0) {
    ...
    findById(id) {
        return new Promise((resolve, reject) => {
            httpGet("/dingdandingdanxinxi/findById", { id }).then((res) => {
                if (res.code == 0) {
                    resolve(res.data);
                } else {
                    reject(res.msg);
    ...
    filter(data) {
        return new Promise((resolve, reject) => {
            httpPost("/dingdandingdanxinxi/filter", data).then(
                (res) => {
                    if (res.code == 0) {
                        resolve(res);
                    } else {
    ...
    query(data) {
        // 分页查询数据
        return new Promise((resolve, reject) => {
            httpPost("/dingdandingdanxinxi/selectPages", data).then(
                (res) => {
                    if (res.code == 0) {
                        resolve(res);
                    } else {
    ...
    update(data) {
        // 更新数据
        return new Promise((resolve, reject) => {
            httpPost("/dingdandingdanxinxi/update", data).then(
                (res) => {
                    if (res.code == 0) {
                        if (content.state.status == 0) {
    ...
    insert(data) {
        // 插入数据
        return new Promise((resolve, reject) => {
            httpPost("/dingdandingdanxinxi/insert", data).then(
                (res) => {
                    if (res.code == 0) {
                        if (content.state.status == 0) {
                            content.dispatch("selectAll");
    ...
    checkForm(data) {
        return new Promise((resolve, reject) => {
            ...
            rules.kucun = [{ validator: rule.checkNumber, message: "输入一个有效数字" }];

            rules.jiage = [{ validator: rule.checkNumber, message: "输入一个有效数字" }];

            rules.goumaishuliang = [
                { validator: rule.checkNumber, message: "输入一个有效数字" },
                { validator: rule.checkMin, message: "最小为1", value: 1 },
                { validator: rule.checkMax, message: "库存不足", value: readMap.kucun },
            ];
    ...
    delete(data) {
        // 删除数据
        return new Promise((resolve, reject) => {
            httpPost("/dingdandingdanxinxi/delete", isArray(data) ? data : [data]).then(
                (res) => {
                    if (res.code == 0) {
                        if (content.state.status == 0) {
                            content.dispatch("selectAll");
