...
var content = (actions = {
    selectAll() { // 获取所有数据
        return new Promise((resolve, reject) => {
            if (content.state.status == 0) {
                ...
                httpGet("/gouwuche/selectAll").then(
                    (res) => {
                        if (res.code == 0) {
    ...
    findById(id) {
        return new Promise((resolve, reject) => {
            httpGet("/gouwuche/findById", { id }).then((res) => {
                if (res.code == 0) {
                    resolve(res.data);
                } else {
                    reject(res.msg);
    ...
    filter(data) {
        return new Promise((resolve, reject) => {
            httpPost("/gouwuche/filter", data).then(
                (res) => {
                    if (res.code == 0) {
                        resolve(res);
                    } else {
    ...
    query(data) {
        // 分页查询数据
        return new Promise((resolve, reject) => {
            httpPost("/gouwuche/selectPages", data).then(
                (res) => {
                    if (res.code == 0) {
                        resolve(res);
                    } else {
    ...
    querygoumairen(data) {
        // 分页查询数据
        return new Promise((resolve, reject) => {
            httpPost("/gouwuche/selectPagesGoumairen", data).then(
                (res) => {
                    if (res.code == 0) {
                        resolve(res);
                    } else {
    ...
    update(data) {
        // 更新数据
        return new Promise((resolve, reject) => {
            httpPost("/gouwuche/update", data).then(
                (res) => {
                    if (res.code == 0) {
                        if (content.state.status == 0) {
    ...
    insert(data) {
        // 插入数据
        return new Promise((resolve, reject) => {
            httpPost("/gouwuche/insert", data).then(
                (res) => {
                    if (res.code == 0) {
                        if (content.state.status == 0) {
                            content.dispatch("selectAll");
    ...
    delete(data) {
        // 删除数据
        return new Promise((resolve, reject) => {
            httpPost("/gouwuche/delete", isArray(data) ? data : [data]).then(
                (res) => {
                    if (res.code == 0) {
                        if (content.state.status == 0) {
                            content.dispatch("selectAll");
