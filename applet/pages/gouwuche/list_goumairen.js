...
Page({
    ...
    onChangeNumber(e) {
        ...
        var vo = list[index];
        vo.goumaishuliang = value;

        this.setDataObj("gouwuchelist[" + index + "].goumaishuliang", value);
        const st = app.include("/request/module/gouwuche");
        // 这样更新数量 会出问题，会把数据上传上去,只需要提交两个字段就可以了
        var dat = {
            id: vo.id,
            goumaishuliang: vo.goumaishuliang,
        };
    ...
    calcPrice(list) {
        ...
        for (var i in list) {
            var ci = list[i];
            result += parseFloat(ci.jiage) * parseInt(ci.goumaishuliang);
        }
        this.setData({
